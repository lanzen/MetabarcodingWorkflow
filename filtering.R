...
dropRareByAvgAbundance = function(abundances, minRelativeAbundance, 
                                  taxaAsRows = FALSE) {
  require(vegan)
  # Transpose if taxa are rows
  if (taxaAsRows) abundances = as.data.frame(t(abundances))
  
  relativeAbundances = decostand(abundances, method="total")
  
  # Filter and return
  avgTaxonAbundances = colMeans(relativeAbundances)
  
  filtered = abundances[,avgTaxonAbundances >= minRelativeAbundance]
  if (taxaAsRows) filtered = as.data.frame(t(filtered))
  return(filtered)
}
...
# Prevalence filter returning smaller OTUs table df with at least X % (default 10) 
# prevalence discarding other OTUs
prevFilter = function(otus, minPrev = 0.10) {
  otus.pa = decostand(otus,"pa")
  minN = dim(otus)[1]*minPrev
  return(otus[,colSums(otus.pa)>=minN])
}